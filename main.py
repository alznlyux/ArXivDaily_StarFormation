...
def main(args):
    ...
    for i in range(len(dt_list)):
        ...
        a_abs = dt_list[i].find("a", title="Abstract")
        paper_number = a_abs["href"].rstrip("/").split("/")[-1]  # e.g. 2501.01234
        paper['main_page'] = f"https://arxiv.org/abs/{paper_number}"
        paper['pdf']       = f"https://arxiv.org/pdf/{paper_number}.pdf"
    ...
    def send_email_with_md(md_path: str, subject_hint: str = None):
        ...
        html_body = markdown.markdown(
            text,
            extensions=[
                "extra", "nl2br", "sane_lists", "toc",
                "pymdownx.magiclink"   # ← 这行让裸 URL 自动变链接
            ]
        )
        ...
        m_file = re.match(r'(\d{4}-\d{2}-\d{2})-Arxiv-Daily', bn)
        if m_file:
            subject = f"arXiv Daily · {m_file.group(1)} · GA/SR"
        else:
            # 尝试从正文找第一个二/三级标题
            m_h = re.search(r'^\s*#{2,3}\s*(.+)$', text, flags=re.M)
            if m_h:
                subject = f"arXiv Daily · {m_h.group(1).strip()}"
            else:
                subject = f"arXiv Daily · {_dt.date.today().isoformat()}"

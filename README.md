...
Create a new GitHub repository and copy the standalone AstroBrief files into it. Inside GitHub Actions, the code detects `GITHUB_REPOSITORY` automatically, so the repository owner/name do not need to be hard-coded.
...
The semantic recommendation pipeline, official arXiv ingestion, GitHub Actions automation, score archiving, GitHub Issue generation, and SMTP delivery have been exercised in the development repository. The `standalone-v1` branch is the release candidate being cleaned for migration into the independent **AstroBrief** repository.

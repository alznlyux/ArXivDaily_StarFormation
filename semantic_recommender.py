# coding: utf-8
"""Free semantic recommender for the group ISM arXiv digest.

Pipeline:
  SPECTER2 scientific embeddings
  -> positive-vs-negative semantic contrast
  -> explicit ISM-object evidence / category gate
  -> small local zero-shot NLI reranker
  -> stable group-scope calibration
...
def _gate_priority(p: dict, priority: str, pos: float, neg: float, dscore: float, title_dscore: float, negative_name: str) -> tuple[str, str]:
    ...
    if primary in SECONDARY_ASTRO and dscore < 2.5 and priority in {"A", "B"}:
        if margin >= 0.035 and dscore >= 1.0:
            priority, reason = "C", "secondary astro category with weak ISM-object evidence"
        else:
            return "SKIP", "secondary astro category without direct ISM-object evidence"
    ...
    if strong_negative and dscore < 2.0 and neg >= pos - 0.020:
        return "SKIP", "negative-domain match with weak ISM-object evidence"

    if priority in {"SKIP", "C"}:
        if _direct_title(p["title"]) and dscore >= 4.0 and pos >= 0.70 and margin >= -0.025:
            priority, reason = "B", "rescued by direct ISM/star-formation title evidence"
        elif dscore >= 6.0 and pos >= 0.71 and margin >= -0.020:
            priority, reason = "C", "rescued by strong multi-signal ISM evidence"

    if priority == "A" and dscore < 1.5 and title_dscore == 0:
        priority, reason = "C", "A capped: insufficient direct ISM-object evidence"
...
def _nli_decision(p: dict, pos_score: float, neg_score: float) -> tuple[str, str]:
    ...
    if old in {"A", "B"}:
        if share < 0.32 and not strong_object:
            return "SKIP", "NLI strongly favors a non-ISM domain"
        if share < 0.48 and not strong_object:
            return _one_down(old), "NLI weakens the ISM interpretation"
        if share < 0.60 and dscore < 4.0 and title_d == 0.0:
            return _one_down(old), "weak NLI ISM preference without direct object evidence"
    ...
    if old == "C":
        if share >= 0.72 and (title_d >= 3.0 or dscore >= 6.0) and primary in {"astro-ph.GA", "astro-ph.SR", "astro-ph.HE"}:
            return "B", "NLI promotes a strongly evidenced ISM candidate"
    ...
    if old == "SKIP":
        if share >= 0.78 and title_d >= 3.0 and primary in {"astro-ph.GA", "astro-ph.SR", "astro-ph.HE"}:
            return "B", "NLI rescues a title-confirmed ISM near-miss"
        if share >= 0.70 and (title_d >= 3.0 or (primary in {"astro-ph.GA", "astro-ph.SR"} and dscore >= 6.0)):
            return "C", "NLI rescues a conservative ISM near-miss"
...
def _scope_calibrate(p: dict) -> tuple[str, str]:
    ...
    explicit_hi_title = re.search(r"\b(?:neutral gas|neutral hydrogen|H\s*I\s+(?:clouds?|gas|emission|absorption|survey))\b", title, flags=re.I) is not None
    ...
    external_cgm = h(r"\b(?:circumgalactic medium|CGM)\b") and not h(r"\b(?:Milky Way|Galactic Cent(?:re|er)|Fermi Bubbles?)\b")
    if external_cgm and old in {"A", "B"}:
        return "C", "scope cap: external-galaxy CGM"

    highz = h(r"\b(?:high[- ]redshift|early Universe|reionization|z\s*[=~>]\s*[4-9])\b") and re.search(r"\bgalax(?:y|ies)\b", title, flags=re.I)
    if highz and old in {"A", "B"}:
    ...
    direct_ism_title = re.search(
        r"\b(?:molecular cloud|interstellar medium|neutral hydrogen|H\s*I\s+(?:gas|cloud|emission)|dense core|H\s*II region|star[- ]forming region)\b",
        title, flags=re.I,
    ) is not None
    if plural_galaxies and not direct_ism_title and old in {"A", "B"}:
...
def score_papers(papers: list[dict], config_path: str | Path = "semantic_topics.json") -> tuple[list[dict], dict]:
    ...
    for i, p in enumerate(papers):
        pi = int(np.argmax(pos_sem[i])); ni = int(np.argmax(neg_sem[i]))
        pos = float(pos_sem[i, pi]); neg = float(neg_sem[i, ni]); lex = float(np.max(pos_lex[i]))
    ...
    shortlist = [i for i, p in enumerate(results) if p["priority"] != "SKIP" or p["base_priority"] in {"A", "B"} or p["domain_evidence_score"] >= 4.0 or p["title_domain_score"] >= 3.0]
    if shortlist:
        nli_model, nli_tok = _load_nli()
        pos_labels = cfg["nli_positive_labels"]; neg_labels = cfg["nli_negative_labels"]
        labels = pos_labels + neg_labels
        premises = ["Title: " + results[i]["title"] + "\nAbstract: " + results[i]["abstract"] for i in shortlist]
        scores = _nli_scores(nli_model, nli_tok, premises, labels)
        npos = len(pos_labels)
        for local_i, paper_i in enumerate(shortlist):
            p = results[paper_i]
            ps = scores[local_i, :npos]; ns = scores[local_i, npos:]
            pidx = int(np.argmax(ps)); nidx = int(np.argmax(ns))
            pscore = float(ps[pidx]); nscore = float(ns[nidx])
